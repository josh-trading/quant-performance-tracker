...
const getStrategyCache = async () => {  
  ...
  const params = {
    TableName: param.xrpFireOkxTest.table.cache,
    Key: {
      strategy: param.xrpFireOkxTest.name
    },
  };
    ...
    if (Object.keys(parentOrder).length === 1) {
      ...
      if (strategyCache.status == 'open') {
        const getPosition = async () => {
          try {
              const position = await account.info.getPositionList(accountOKX, "copyTrade", {
                instId: [param.xrpFireOkxTest.order.symbol, param.xrpFireOkxTest.order.insType].join('-')
              });
        ...
        const symbolPrice = (await market.info.getTickerInfo(
          accountOKX,
          "future",
          {instType: param.xrpFireOkxTest.order.insType, instId: param.xrpFireOkxTest.order.symbol}
        )).data[0].last;
