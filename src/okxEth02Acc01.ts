...
const getStrategyCache = async () => {  
  ...
  const params = {
    TableName: param.eth02OkxTest.table.cache,
    Key: {
      strategy: param.eth02OkxTest.name
    },
  };
    ...
    if (Object.keys(parentOrder).length === 1) {
      ...
      if (strategyCache.status == 'open') {
        const getPosition = async () => {
          try {
              const position = await account.info.getPositionList(accountOKX, "copyTrade", {
                instId: [param.eth02OkxTest.order.symbol, param.eth02OkxTest.order.insType].join('-')
              });
        ...
        const symbolPrice = (await market.info.getTickerInfo(
          accountOKX,
          "future",
          {instType: param.eth02OkxTest.order.insType, instId: param.eth02OkxTest.order.symbol}
        )).data[0].last;
