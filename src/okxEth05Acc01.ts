...
const getStrategyCache = async () => {  
  ...
  const params = {
    TableName: param.eth05OkxTest.table.cache,
    Key: {
      strategy: param.eth05OkxTest.name
    },
  };
    ...
    if (Object.keys(parentOrder).length === 1) {
      ...
      if (strategyCache.status == 'open') {
        const getPosition = async () => {
          try {
              const position = await account.info.getPositionList(accountOKX, "copyTrade", {
                instId: [param.eth05OkxTest.order.symbol, param.eth05OkxTest.order.insType].join('-')
              });
        ...
        const symbolPrice = (await market.info.getTickerInfo(
          accountOKX,
          "future",
          {instType: param.eth05OkxTest.order.insType, instId: param.eth05OkxTest.order.symbol}
        )).data[0].last;
