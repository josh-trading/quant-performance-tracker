...
const getStrategyCache = async () => {  
  ...
  const params = {
    TableName: param.ethExplodeOkxTest.table.cache,
    Key: {
      strategy: param.ethExplodeOkxTest.name
    },
  };
    ...
    if (Object.keys(parentOrder).length === 1) {
      ...
      if (strategyCache.status == 'open') {
        const getPosition = async () => {
          try {
              const position = await account.info.getPositionList(accountOKX, "copyTrade", {
                instId: [param.ethExplodeOkxTest.order.symbol, param.ethExplodeOkxTest.order.insType].join('-')
              });
        ...
        const symbolPrice = (await market.info.getTickerInfo(
          accountOKX,
          "future",
          {instType: param.ethExplodeOkxTest.order.insType, instId: param.ethExplodeOkxTest.order.symbol}
        )).data[0].last;
