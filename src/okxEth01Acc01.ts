...
const getStrategyCache = async () => {  
  ...
  const params = {
    TableName: param.eth01OkxTest.table.cache,
    Key: {
      strategy: param.eth01OkxTest.name
    },
  };
    ...
    if (Object.keys(parentOrder).length === 1) {
      ...
      if (strategyCache.status == 'open') {
        const getPosition = async () => {
          try {
              const position = await account.info.getPositionList(accountOKX, "copyTrade", {
                instId: [param.eth01OkxTest.order.symbol, param.eth01OkxTest.order.insType].join('-')
              });
        ...
        const symbolPrice = (await market.info.getTickerInfo(
          accountOKX,
          "future",
          {instType: param.eth01OkxTest.order.insType, instId: param.eth01OkxTest.order.symbol}
        )).data[0].last;
