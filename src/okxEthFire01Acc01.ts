...
const getStrategyCache = async () => {  
  ...
  const params = {
    TableName: param.ethFire01OkxTest.table.cache,
    Key: {
      strategy: param.ethFire01OkxTest.name
    },
  };
    ...
    if (Object.keys(parentOrder).length === 1) {
      ...
      if (strategyCache.status == 'open') {
        const getPosition = async () => {
          try {
              const position = await account.info.getPositionList(accountOKX, "copyTrade", {
                instId: [param.ethFire01OkxTest.order.symbol, param.ethFire01OkxTest.order.insType].join('-')
              });
        ...
        const symbolPrice = (await market.info.getTickerInfo(
          accountOKX,
          "future",
          {instType: param.ethFire01OkxTest.order.insType, instId: param.ethFire01OkxTest.order.symbol}
        )).data[0].last;
